	...
	if(!strcmp(cmd, "/mission", true))
	{
		...
		if(!strlen(tmp))
		{
	 	...
	 	if(!strcmp(tmp, "start", true))
	 	{
			...
			new msg[64];
			format(msg, 64, "You have successfully created a mission (ID: %d)", slot);
	 	...
	 	if(!strcmp(tmp, "invite", true))
	 	{
			...
			format(msg, 128, "%s has invited you to join his mission", name);
			SendClientMessage(strval(tmp), COLOR_GREY, msg);
			format(msg, 128, "Use </mission join %d> to join it.", PlayerMission[playerid]);
			SendClientMessage(strval(tmp), COLOR_GREY, msg);
	 	}
	}
	return 1;
...
stock GetPlayerMissionSlot(playerid)
{
	for(new i = 0; i < MAX_MISSION_PLAYERS; i++)
	{
	    if(Mission[PlayerMission[playerid]][players][i] == playerid)
			return i;
	}
...
public ProcessMissionTimer(mission)
{
	...
	if(Mission[mission][cutscene] && Mission[mission][cutscene_process])
	{
	    ...
	    if(Mission[mission][level] == 0)
	    {
		    ...
		    if(Mission[mission][cutscene_stage] >= Mission[mission][cutscene_stages][Mission[mission][level]])
      			// End the cutscene
	        	EndCutscene(mission);
			else
			{
				...
				if((GetTickCount() - LastCutsceneTick) >= MissionCutsceneInfo[stage][0])
				{
				    ...
				    new msg[190], name[24];
				    GetPlayerName(playerid, name, 24);
				    format(msg, 190, "~g~%s: ~w~%s", name, MissionCutscenes[stage]);
...
public StartMission(slotid, starterid)
{
	...
	for(new i = 0; i < MAX_MISSION_NPCS; i++)
	{
	    // Generate a name
	 	new name[24];
	 	format(name, 24, "mission_%d_%d", slotid, i);
	 	// Create the NPC
	...
	KillTimer(missiontimer[missionid]);
 	// Reset missions
   	...
   	for(new j = 0; j < MAX_MISSION_PLAYERS; j++)
   	{
		...
		{
			new reas[164];
			format(reas, 164, "~r~Mission failed !~n~~w~%s", reason);
...
public JoinMission(playerid, mission, slot)
{
	...
	new msg[64], name[24];
	GetPlayerName(playerid, name, 24);
	format(msg, 64, "~b~%s~n~~w~has joined the mission", name);
  	for(new i = 0; i < MAX_MISSION_PLAYERS; i++)
   	{
...
public LeaveMission(playerid)
{
	...
	Mission[PlayerMission[playerid]][players][slot] = INVALID_PLAYER_ID;
	Mission[PlayerMission[playerid]][player_dead][slot] = false;
	PlayerMission[playerid] = INVALID_MISSION;
	PlayerCreatedMission[playerid] = INVALID_MISSION;
	// Reset player stats
	...
	else
	{
		...
		new msg[64], name[24];
		GetPlayerName(playerid, name, 24);
		format(msg, 64, "~b~%s~n~~w~has left the mission", name);
	  	for(new i = 0; i < MAX_MISSION_PLAYERS; i++)
	   	{
...
public OnPlayerDeath(playerid, killerid, reason)
{
	...
	if(PlayerMission[playerid] != INVALID_MISSION)
	{
		...
		if(spec != INVALID_PLAYER_ID)
		{
			...
			new msg[64], name[24];
			GetPlayerName(playerid, name, 24);
			format(msg, 64, "~b~%s~n~~w~has died", name);
	    	for(new i = 0; i < MAX_MISSION_PLAYERS; i++)
	    	{
   				if(!IsPlayerConnected(Mission[PlayerMission[playerid]][players][i]) || Mission[PlayerMission[playerid]][players][i] == playerid)
		    		continue;
